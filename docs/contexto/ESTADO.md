...
- `/favoritos` · `/carrito` (checkout + cupón) · `/pagina/[slug]` (contenido) · `/soporte` (tickets) · `/pedido` (seguimiento por ref+email: línea recibido→pagado→enviado + nº seguimiento) · `/acceso` (muro)
