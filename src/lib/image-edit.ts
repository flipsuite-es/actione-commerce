...
const STRATEGY_DIRECT =
  ...
  "Relight the WHOLE scene as a white studio: the cushion/prop and the background become clean, bright WHITE with neutral light — remove any warm, grey or yellowish colour cast from them — while keeping the SAME prop with its exact wrinkles and texture, the same framing and camera angle, and the piece's natural soft cast shadow. " +
...
const STRATEGY_WHITE_CUBE =
  ...
  "The whole scene gets the white-cube treatment: the cushion/prop and the background become clean bright white with neutral studio light (no warm, grey or yellowish cast), keeping the same prop with its exact wrinkles, the same framing and a soft natural cast shadow under the piece. " +
  "The piece itself stays identical: exact shape, size, dents and hammered texture, same colour and bright glossy mirror finish (never dull or matte). No gemstones added. Photorealistic.";
